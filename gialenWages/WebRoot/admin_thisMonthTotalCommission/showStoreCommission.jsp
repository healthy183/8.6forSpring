...
<html>
  <head>
    ...
    <title>My JSP 'showStoreCommission.jsp' starting page</title>
 	...
 	jq(function(){
 		...
 		jq("#button").click(function(){
			...
			dwrShowError.uploadExecl(realpath,function(fileString){
					...
					dwrShowError.showStoreCommissionError(fileString,test);	
				});
 		});
 ...
 var jq = jQuery.noConflict();
	jq(function(){
		jq("#operatingMonthId").change(function(){
			location.href
				= "admin_thisMonthTotalCommission_writeStoreCommission.action?operatingMonthType="+
  ...
  <script language="javascript" charset="gb2312" src="js/validateExcel.js" >
 </script>
  </head>
  
  <body>
  <s:form 
		action="admin_thisMonthTotalCommission_writeStoreCommissionSuccess.action"
		 	namespace="/admin_thisMonthTotalCommission" enctype="multipart/form-data"
		 		method="post" id="upLoadForm" theme="simple">
		
		<table class="myTable" cellpadding="3" cellspacing="1">
    		<tr>
    			<th class="myTableTh" align="left">
					      <s:hidden name="operatingMonthPathMoneyType" value="%{#parameters.operatingMonthPathMoneyType}"></s:hidden>
    					  <s:hidden name="operatingMonthType" value="%{#parameters.operatingMonthType}"></s:hidden>
					   <b style="font-size:16px;"> 
					      如需更新,请重新上传 <select name="operatingMonthId" id="operatingMonthId">
						<option
    	...
    	<s:iterator value="#request.planMoneyList" var="planMoney">
    		<tr class="mybgcolor"><td><s:property value="operatingMonthPlanMoneyName"></s:property></td>
    			<s:iterator value="#planMoney.grundbonuses">
    				<td>
    					<s:property value="grundbonusName"></s:property>
    				</td>
    			</s:iterator>
    		</tr>
    	</s:iterator>
